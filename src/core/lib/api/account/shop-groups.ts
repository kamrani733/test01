"use server";
import { ShopGroupFormData } from "@/core/schemas/shopGroupSchema";
...
export const getShopGroupsList = async () => {
  await getCsrfToken();
  const { data } = await api.get("api/admin/shop-groups/list");
  return data;
};
...
export const getMetaFieldsByGroupId = async (id: number) => {
  try {
    await getCsrfToken();
    const { data } = await api.get(`api/admin/shop-groups/${id}/meta-fields`);
    return data;
  } catch (error) {
    if (axios.isAxiosError(error)) {
      const errData = error.response?.data;
