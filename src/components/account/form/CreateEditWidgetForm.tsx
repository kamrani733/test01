...
import { createWidget, updateWidget } from '@/core/lib/api/account/widgets';
import { Widget, WidgetTemplate } from '@/core/models/widget-model';
import {
  getWidgetFormSchema,
  WidgetFormData,
} from '@/core/schemas/widgetSchema';
import { Options } from '@/core/types/options';

import AddItemSelect from '../AddItemSelect';
import ContainerView from '../ContainerView';
import FormSection from '../FormSection';
// Import the TextareaInput component
...
export default function CreateEditWidgetForm({
  ...
  // Define multilingual fields explicitly
  ...
  type MultilingualField = (typeof multilingualFields)[number];
  ...
  const isDescriptionField = (field: string) =>
    field === 'description' ||
        ...
        {template ? (
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 w-full">
            ...
            <FormSection title={dictionary.ui.form.widgetInfo}>
              {multilingualMainGroups.map((baseField: MultilingualField) => (
                <Tabs defaultValue="fa" className="w-full" key={baseField}>
                  ...
                  <TabsContent value="fa">
                    {fields.main.includes(baseField) &&
                      (isDescriptionField(baseField) ? (
                        <TextareaInput
                          label={getLabelOrPlaceholder(baseField, 'fa')}
                          name={baseField as Path<WidgetFormData>}
                  ...
                  {fields.main.includes(`${baseField}_en`) && (
                    <TabsContent value="en">
                      <TextareaInput
                        ...
                        name={`${baseField}_en` as Path<WidgetFormData>}
                  ...
                  {fields.main.includes(`${baseField}_ar`) && (
                    <TabsContent value="ar">
                      <TextareaInput
                        ...
                        name={`${baseField}_ar` as Path<WidgetFormData>}
                        placeholder={getLabelOrPlaceholder(
                          `${baseField}_ar`,
                          'ar'
                        )}
              ...
              <>
                {items.map((item, index) => (
                  <FormSection
                    ...
                    setValue={setValue}
                    defaultItems={defaultValue?.items}
                    onRemove={
                      items.length > 1 ? () => remove(index) : undefined
                    }
                  >
                    {multilingualItemsGroups.map(
                      (baseField: MultilingualField) => (
                        <Tabs
                          defaultValue="fa"
                          className="w-full"
                            ...
                            {fields.items.includes(baseField) &&
                              (isDescriptionField(baseField) ? (
                                <TextareaInput
                                  label={getLabelOrPlaceholder(baseField, 'fa')}
                                  name={
                                    `items.${index}.${baseField}` as Path<WidgetFormData>
                                  }
                                  placeholder={getLabelOrPlaceholder(
                                    baseField,
                                    'fa'
                          ...
                          {fields.items.includes(`${baseField}_en`) && (
                            <TabsContent value="en">
                              <TextareaInput
                                ...
                                name={
                                  `items.${index}.${baseField}_en` as Path<WidgetFormData>
                                }
                                placeholder={getLabelOrPlaceholder(
                                  `${baseField}_en`,
                                  'en'
                          ...
                          {fields.items.includes(`${baseField}_ar`) && (
                            <TabsContent value="ar">
                              <TextareaInput
                                ...
                                name={
                                  `items.${index}.${baseField}_ar` as Path<WidgetFormData>
                                }
                                placeholder={getLabelOrPlaceholder(
                                  `${baseField}_ar`,
                                  'ar'
